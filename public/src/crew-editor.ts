import { BoatSize, CrewMemberInterface, CrewMember } from './types';
import {
	CrewEditorItem,
	CrewEditorSerialised,
} from './components/CrewEditorItem';
...
class CrewEditor {
	...
	constructor(querySelector: string) {
		...
		this.editor.addEventListener('returningCrewMember', () => {
			this.removeMemberFromUsedList(
				parseInt(window.sessionStorage.getItem('returningCrewMember')!)
			);
		...
		this.editor.addEventListener('acceptedCrewMember', () => {
			this.addMemberToUsedList(
				parseInt(window.sessionStorage.getItem('acceptedCrewMember')!)
			);
		...
		this.editor.addEventListener('delete', () => {
			this.deleteCrew(
				parseInt(window.sessionStorage.getItem('crewToDelete'))
			);
		...
		this.fabSelectButtons.forEach((val: HTMLElement, index: number) => {
			val.onclick = () => {
				...
				this.crews.push(
					new CrewEditorItem({
						parent: this.editor,
						config: { size: buttonCrewSizeValue, coxed: coxed },
					})
				);
				this.saveAllCrews();
				this.renderAllCrews();
	...
	private saveAllCrews() {
		...
		window.localStorage.setItem(
			'crews',
			btoa(JSON.stringify(serialisedOject))
		);
	...
	private loadAllCrews() {
		...
		this.crews = [];
		// let decodedLocalStorage = JSON.parse(
		...
		// 	(val.seats as number[]).forEach((val2) => {
		// 		if (val2 == null || val2 == undefined) return;
		// 		this.usedCrewMembers.push(val2.toString());
		// 	});

		// 	this.crews.push(
		// 		new CrewEditorItem({ parent: this.editor, serialisedForm: val })
		// 	);
		// });

		fetch('/api/crews').then(async (val: Response) => {
			let data = (await val.json()) as Crew[];
	...
	private addMemberToUsedList(id: number) {
		if (this.usedCrewMembers.indexOf(id + '') == -1)
			this.usedCrewMembers.push(id.toString());
	...
	private loadCrewMembers() {
		...
		return fetch('/api/crews/').then(async (val) => {
			const val2 = await val.json();
			// return val2;
			this.crews = (val2 as CrewInterface[]).map((cr) => {
				return new CrewEditorItem({
					parent: this.editor,
					serialisedForm: {
						boatName: cr.boatName || '',
						coachName: cr.coach || '',
						oars: cr.oars || '',
						crewType: cr.crewName || '',
						seats: cr.seats || ([] as number[]),
					} as CrewEditorSerialised,
				});
			});
			this.insertDataIntoTable();
